...
int32_t CRabbitMQ_Adapter::Connect(string &ErrorReturn)
{
	...
	if (1 == AssertError(amqp_login(m_conn, "/", 0, 131072, 0, AMQP_SASL_METHOD_PLAIN, m_user.c_str(), m_psw.c_str()), "Loging in", ErrorReturn))
		return 0;
...
int32_t CRabbitMQ_Adapter::exchange_declare(CExchange &exchange, string &ErrorReturn)
{
	...
	amqp_exchange_declare(m_conn, m_channel, _exchange, _type, _passive, _durable, amqp_empty_table);
...
int32_t CRabbitMQ_Adapter::AssertError(amqp_rpc_reply_t x, string context, string &ErrorReturn)
{
	...
	case AMQP_RESPONSE_NONE:
		sprintf_s(rtnmsg, "%s: missing RPC reply type!\n", context.c_str());
		break;

	case AMQP_RESPONSE_LIBRARY_EXCEPTION:
		sprintf_s(rtnmsg, "%s: %s\n", context.c_str(), amqp_error_string2(x.library_error));
		break;
	...
	case AMQP_RESPONSE_SERVER_EXCEPTION:
		...
		case AMQP_CONNECTION_CLOSE_METHOD: {
			...
			sprintf_s(rtnmsg, "%s: server connection error %d, message: %.*s\n",
				context.c_str(),
				m->reply_code,
		...
		case AMQP_CHANNEL_CLOSE_METHOD: {
			...
			sprintf_s(rtnmsg, "%s: server channel error %d, message: %.*s\n",
				context.c_str(),
				m->reply_code,
				(int)m->reply_text.len, (char *)m->reply_text.bytes);
			break;
		...
		default:
			sprintf_s(rtnmsg, "%s: unknown server error, method id 0x%08X\n", context.c_str(), x.reply.id);
			break;
		}
		break;
	}
